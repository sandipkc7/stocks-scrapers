...
def parse_record(raw, price_type):
    ...
    try:
        ...
        symbol = row.get("symbol") or row.get("stock_symbol")
        if not date or not symbol:
            return None
            
        parsed = {"date": date, "symbol": symbol}
        if price_type == "adjusted":
...
def fetch_by_date(session, token, specific_date, price_type="adjusted"):
    """POST to /nepse-data for a specific date (all symbols). Returns (records, status)."""
    headers = {
    ...
    if resp.status_code in (419, 403):
        log.warning("  Auth error HTTP %d for specific date %s", resp.status_code, specific_date)
...
def upsert_to_db_by_date(records, price_type):
    """Upsert adjusted or unadjusted OHLC columns for multiple symbols."""
    if not records:
        return 0
    ...
    if price_type == "adjusted":
        tuples = [
            (r["date"], r["symbol"], r["adj_open"], r["adj_high"], r["adj_low"], r["adj_close"])
            for r in records
        ]
        sql = """
            INSERT INTO daily_price (date, symbol, adj_open, adj_high, adj_low, adj_close)
        ...
        tuples = [
            (r["date"], r["symbol"], r["open"], r["high"], r["low"], r["close"], r.get("traded_shares"), r.get("turnover"), r.get("previous_close"))
            for r in records
        ]
        sql = """
            INSERT INTO daily_price (date, symbol, open, high, low, close, traded_shares, turnover, previous_close)
...
def scrape_by_date(session, token, specific_date, price_types):
    """Fetch + upsert all symbols for a specific date. Returns row count."""
    total_upserted = 0
    for p_type in price_types:
        for attempt in range(1, MAX_RETRIES + 1):
            raw_list, status = fetch_by_date(session, token, specific_date, price_type=p_type)

            if status == "ok":
                parsed = [p for r in raw_list if (p := parse_record(r, p_type)) is not None]
                if not parsed:
                    log.warning("  [%s][%s] Data returned but unparseable.", specific_date, p_type)
                    break
                n = upsert_to_db_by_date(parsed, p_type)
                log.info("  [%s][%s] Upserted %d rows.", specific_date, p_type, n)
                total_upserted += n
                break

            if status == "empty":
                log.info("  [%s][%s] No data found.", specific_date, p_type)
                break

            if status == "auth_expired":
                log.error("  [%s][%s] Auth expired (403/419). Re-run to refresh credentials.", specific_date, p_type)
                break

            if attempt < MAX_RETRIES:
                log.info("  Retry %s [%s] in %ds...", specific_date, p_type, RETRY_DELAY)
                time.sleep(RETRY_DELAY)
            else:
                log.error("  [%s][%s] Failed after %d attempts (%s).", specific_date, p_type, MAX_RETRIES, status)
                
    return total_upserted


def main():
    parser = argparse.ArgumentParser(description="NepseAlpha Adjusted & Unadjusted OHLC Scraper (By Date)")
    ...
    print("=" * 60)
    print("   NepseAlpha OHLC Scraper (Specific Date)")
    print("=" * 60)

    import sys
    if sys.stdin.isatty():
        specific_date = args.date or input(f"Specific date (YYYY-MM-DD) [default {today}]: ").strip() or today
    else:
        specific_date = args.date or today
        
    log.info("Target date: %s", specific_date)
    ...
    # Step 3: Fetch for the specific date
    total = scrape_by_date(session, csrf_token, specific_date, price_types)

    print("\\n" + "=" * 60)
    print(f"   Done. {total} rows upserted for {specific_date}.")
    print("=" * 60)
